...
static gboolean find_edit_cb(GtkWidget *widget, GdkEventKey *ev, gpointer *data)
{
    (void)data;
...
static gint scroll_event(GtkWidget *widget, GdkEventScroll *event, gpointer text)
{
    (void)widget;
    ...
    switch (event->direction) {
        case GDK_SCROLL_UP:
            ...
            if (gtk_adjustment_get_value(vadj) < gtk_adjustment_get_lower(vadj))
        ...
        case GDK_SCROLL_DOWN:
            gtk_adjustment_set_value(vadj, gtk_adjustment_get_value(vadj) + s_val);
            if (gtk_adjustment_get_value(vadj) > gtk_adjustment_get_upper(vadj) - p_val)
                gtk_adjustment_set_value(vadj, gtk_adjustment_get_upper(vadj) - p_val);
...
static gint destroy_via_closebutton_release(GtkWidget *widget, GdkEventButton *event)
{
    ...
    if ((event->x < 0) || (event->x >= allocation.width) || (event->y < 0) ||
